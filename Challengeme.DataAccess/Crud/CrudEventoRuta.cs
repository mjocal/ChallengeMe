...
{
    public class CrudEventoRuta : CrudFactory
    {
        public CrudEventoRuta()
        {
            _mapper = new EventoRutaMapper();

        }
        private readonly EventoRutaMapper _mapper;
        ...
        public override T Retrieve<T>(BaseEntity entity)
        {
            ...
            //en la BD
            var userEncontrado = _mapper.BuildObject(list);//contruye el objeto pasandole los datos
                                                           //a cada atributo
            try
            {
                if (lstResult.Count <= 0) return default(T);
                list = lstResult[0];
                userEncontrado = _mapper.BuildObject(list);
            ...
            return (T)Convert.ChangeType(userEncontrado, typeof(T));
