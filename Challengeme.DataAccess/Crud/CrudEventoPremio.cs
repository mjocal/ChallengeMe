...
{
    public class CrudEventoPremio : CrudFactory
    {
        public CrudEventoPremio()
        {
            _mapper = new EventoPremioMapper();

        }
        private readonly EventoPremioMapper _mapper;
        ...
        public override T Retrieve<T>(BaseEntity entity)
        {
            ...
            //en la BD
            var eventoPremioEncontrado = _mapper.BuildObject(list);//contruye el objeto pasandole los datos
                                                                 //a cada atributo
            try
            {
                if (lstResult.Count <= 0) return default(T);
                list = lstResult[0];
                eventoPremioEncontrado = _mapper.BuildObject(list);
            ...
            return (T)Convert.ChangeType(eventoPremioEncontrado, typeof(T));
