...
{
    public class CrudPatrocinadorProducto:CrudFactory
    {
        public CrudPatrocinadorProducto()
        {
            _mapper = new PatrocinadorProductoMapper();

        }
        private readonly PatrocinadorProductoMapper _mapper;
        ...
        public override T Retrieve<T>(BaseEntity entity)
        {
            ...
            //en la BD
            var patrocinadorProductoEncontrado = _mapper.BuildObject(list);//contruye el objeto pasandole los datos
                                                                 //a cada atributo
            try
            {
                if (lstResult.Count <= 0) return default(T);
                list = lstResult[0];
                patrocinadorProductoEncontrado = _mapper.BuildObject(list);
            ...
            return (T)Convert.ChangeType(patrocinadorProductoEncontrado, typeof(T));
        ...
        public override List<T> RetrieveAllById<T>(BaseEntity entity)
        {
            ...
            {
                var instance = SqlDao.GetInstance();
                var lstResult = instance.ExecuteQueryProcedure(_mapper.GetRetriveAllByIdPatrocinador(entity));
