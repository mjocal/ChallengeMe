...
{
   public class CrudInscripcion:CrudFactory
    {
        public CrudInscripcion()
        {
            _mapper = new InscripcionMapper();

        }
        private readonly InscripcionMapper _mapper;
        ...
        public override T Retrieve<T>(BaseEntity entity)
        {
            ...
            //en la BD
            var inscripcionEncontrada = _mapper.BuildObject(list);
            ...
            try
            {
                if (lstResult.Count <= 0) return default(T);
                list = lstResult[0];
                inscripcionEncontrada = _mapper.BuildObject(list);
            ...
            //adecuada
            return (T)Convert.ChangeType(inscripcionEncontrada, typeof(T));
        }
