...
{
    public class CrudEventoPatrocinador : CrudFactory
    {
        public CrudEventoPatrocinador()
        {
            _mapper = new EventoPatrocinadorMapper();

        }
        private readonly EventoPatrocinadorMapper _mapper;
        ...
        public override T Retrieve<T>(BaseEntity entity)
        {
            ...
            //en la BD
            var eventoPatrocinadorEncontrado = _mapper.BuildObject(list);//contruye el objeto pasandole los datos
                                                                 //a cada atributo
            try
            {
                if (lstResult.Count <= 0) return default(T);
                list = lstResult[0];
                eventoPatrocinadorEncontrado = _mapper.BuildObject(list);
            ...
            return (T)Convert.ChangeType(eventoPatrocinadorEncontrado, typeof(T));
        ...
        public override List<T> RetrieveAllById<T>(BaseEntity entity)
        {
            ...
            {
                var instance = SqlDao.GetInstance();
                var lstResult = instance.ExecuteQueryProcedure(_mapper.GetRetriveAllByIdRol(entity));
