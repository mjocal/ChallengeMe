...
{
    public class CrudRutaEvento : CrudFactory
    {
        public CrudRutaEvento()
        {
            _mapper = new RutaMapper();

        }

        private readonly RutaMapper _mapper;
        ...
        public  override List<T> RetrieveAllRutasByIdEvento<T>(int idEvento)
        {
            ...
            var instance = SqlDao.GetInstance();
            var lstResult = instance.ExecuteQueryProcedure(_mapper.GetRetriveAllRoutesByIdEvento(idEvento));
