...
{
    public class CrudEventoActividad : CrudFactory
    {
        public CrudEventoActividad()
        {
            _mapper = new EventoActividadMapper();

        }
        private readonly EventoActividadMapper _mapper;
        ...
        public override T Retrieve<T>(BaseEntity entity)
        {
            ...
            //en la BD
            var eventoActividadEncontrado = _mapper.BuildObject(list);//contruye el objeto pasandole los datos
                                                                 //a cada atributo
            try
            {
                if (lstResult.Count <= 0) return default(T);
                list = lstResult[0];
                eventoActividadEncontrado = _mapper.BuildObject(list);
            ...
            return (T)Convert.ChangeType(eventoActividadEncontrado, typeof(T));
