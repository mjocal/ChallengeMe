...
{
   public class CrudRolPermiso : CrudFactory
    {
        public CrudRolPermiso()
        {
            _mapper = new RolPermisoMapper();

        }
        private readonly RolPermisoMapper _mapper;
        ...
        public override T Retrieve<T>(BaseEntity entity)
        {
            ...
            //en la BD
            var rolPermisoEncontrado = _mapper.BuildObject(list);//contruye el objeto pasandole los datos
                                                                 //a cada atributo
            try
            {
                if (lstResult.Count <= 0) return default(T);
                list = lstResult[0];
                rolPermisoEncontrado = _mapper.BuildObject(list);
            ...
            return (T)Convert.ChangeType(rolPermisoEncontrado, typeof(T));
