...
{
   public class PatrocinadorProductoManagement: BaseManagement
    {
        ...
        public List<T> RetrieveAlls<T>()
        {
            ...
            try
            {
                var lista = _factory.RetrieveAll<PatrocinadorProducto>();
        ...
        public List<T> RetrieveAllByIdPatrocinador<T>(PatrocinadorProducto patrocinadorProducto )
        {
            var lst = new List<T>();
            try
            {
                var list = _factory.RetrieveAllById<PatrocinadorProducto>(patrocinadorProducto);
                if (list.Count > 0)
                {
                    foreach (var obj in list)
                    {
        ...
        public void Delete(PatrocinadorProducto patrocinadorProducto)
        {
            try
            {
                _factory.Delete(patrocinadorProducto);
            }
            catch (Exception ex)
            {
        ...
        public void Update(PatrocinadorProducto patrocinadorProducto)
        {
            try
            {
                _factory.Update(patrocinadorProducto);
            }
            catch (Exception ex)
            {
