...
{
    public class EventoPremioManagement : BaseManagement
    {
        public EventoPremioManagement()
        {
            _factory = new CrudEventoPremio();
        ...
        public List<T> RetrieveAlls<T>()
        {
            ...
            try
            {
                var lista = _factory.RetrieveAll<EventoPremio>();
        ...
        public List<T> RetrieveAllById<T>(EventoPremio eventoPremio)
        {
            var lst = new List<T>();
            try
            {
                var list = _factory.RetrieveAllById<EventoPremio>(eventoPremio);
                if (list.Count > 0)
                {
                    foreach (var obj in list)
                    {
        ...
        public void Delete(EventoPremio eventoPremio)
        {
            try
            {
                _factory.Delete(eventoPremio);
            }
            catch (Exception ex)
            {
        ...
        public void Update(EventoPremio eventoPremio)
        {
            try
            {
                _factory.Update(eventoPremio);
            }
            catch (Exception ex)
            {
